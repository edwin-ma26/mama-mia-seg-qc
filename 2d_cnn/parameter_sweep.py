...
EPOCHS = 30  # Reduced for parameter sweep
IMG_SIZE = (384, 384)
OUTPUT_DIR = "./parameter_sweep_output"
os.makedirs(OUTPUT_DIR, exist_ok=True)

# Early stopping config
EARLY_STOPPING_PATIENCE = 5  # Reduced for parameter sweep
...
PARAMETER_COMBINATIONS = {
    'weight_decay': [0.0, 0.0001, 0.001, 0.01],  # L2 regularization
    'entropy_reg_weight': [0.0, 0.0001, 0.0005, 0.001, 0.005],  # Entropy regularization
    'dropout_rate': [0.0, 0.1, 0.2, 0.3],  # Dropout rate for main layers
    'attn_dropout_rate': [0.0, 0.1, 0.2, 0.3],  # Dropout rate for attention
}
...
def main():
    ...
    summary_df.to_csv(os.path.join(sweep_dir, "parameter_sweep_summary.csv"), index=False)
    ...
    print("TOP 5 RESULTS:")
    print(f"{'='*60}")
    top_results = sorted(valid_results, key=lambda x: x['best_val_f1'], reverse=True)[:5]
    ...
    print(f"\nParameter sweep completed. Results saved to: {sweep_dir}")
