...
class AllSlicesDataset(Dataset):
    ...
    def process_slice(i, img, mask, augment_fn):
        ...
        if augment_fn:
            batch = {"image": slice_2ch[0][None, ...], "mask": slice_2ch[1][None, ...]}
            aug_batch = augment_fn(batch)
            img_aug = aug_batch["image"][0]
            mask_aug = aug_batch["mask"][0]
            slice_2ch = np.stack([img_aug, mask_aug], axis=0)
    ...
    def __getitem__(self, idx):
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            futures = [executor.submit(self.process_slice, i, img, mask, self.augmentation_transforms if self.augment else None)
                       for i in range(mask.shape[0])]
            slices = [f.result() for f in futures if f.result() is not None]
