...
EPOCHS = 50
IMG_SIZE = (256, 256)  # CHANGED from (384, 384) to (256, 256)
OUTPUT_DIR = "./output"
...
class AllSlicesDataset(Dataset):
    def __init__(self, patient_ids, img_size=IMG_SIZE, augment=False):
        ...
        for pid in patient_ids:
            ...
            # Use _resized_norm.nii.gz and _mask_resized.nii.gz files
            img_path = f"../../preprocessed/{pid}/{pid}_resized_norm.nii.gz"
            mask_path = f"../../preprocessed/{pid}/{pid}_mask_resized.nii.gz"
    ...
    def process_slice(i, img, mask, augment_fn):
        ...
        # No resizing: use original shape
        img_slice_proc = img_slice.astype(np.float32)
        mask_slice_proc = mask_slice.astype(np.float32)
        
        # Add channel dimension → shape becomes (1, H, W)
        img_slice_proc = np.expand_dims(img_slice_proc, axis=0)
        mask_slice_proc = np.expand_dims(mask_slice_proc, axis=0)
        
        if augment_fn:
            # Wrap with MetaTensor to enable tracking
            data_dict = {
                "image": MetaTensor(img_slice_proc.copy()),  # ensure new memory
                "mask": MetaTensor(mask_slice_proc.copy())
            }
            augmented = augment_fn(data_dict)
            img_slice_proc = augmented["image"].numpy()
            mask_slice_proc = augmented["mask"].numpy()
        
        # Remove channel dimension and stack as 2-channel tensor
        img_slice_final = img_slice_proc.squeeze(0)  # (H, W)
        mask_slice_final = mask_slice_proc.squeeze(0)  # (H, W)
        slice_2ch = np.stack([img_slice_final, mask_slice_final], axis=0)  # (2, H, W)
...
def train_model_with_params(params, train_loader, test_loader, run_id, output_dir):
    ...
    if torch.cuda.device_count() > 1:
        print(f"Using {torch.cuda.device_count()} GPUs with DataParallel for slice batch processing.")
    ...
    # Create a new run directory in ./output with incremented id (run_000, run_001, ...)
    output_base_dir = "./output"
    existing_run_dirs = [d for d in os.listdir(output_base_dir) if d.startswith("run_") and d[4:7].isdigit()]
    if existing_run_dirs:
        max_id = max([int(d[4:7]) for d in existing_run_dirs])
        new_run_id = max_id + 1
    else:
        new_run_id = 0
    run_dirname = f"run_{new_run_id:03d}"
    in_training_aug_dir = os.path.join(output_base_dir, run_dirname)
    ...
    for epoch in range(EPOCHS):
        ...
        for patient_slices, patient_label, patient_id in tqdm(train_loader, desc=f"Epoch {epoch+1}/{EPOCHS}"):
            ...
            optimizer.zero_grad()
            # --- Batch forward pass for patient slices with DataParallel ---
            slice_logits_list = []
            slice_attn_list = []
            N = patient_slices.shape[0]
            # Use DataParallel for batch processing if available
            for i in range(0, N, BATCH_SIZE):
                batch_slices = patient_slices[i:i+BATCH_SIZE]
                # DataParallel expects input on default device, so ensure batch_slices is on DEVICE
        ...
        with torch.no_grad():
            for patient_slices, patient_label, patient_id in test_loader:
                ...
                # --- Save top-k attended slices with 4% probability ---
                if random.random() < 0.04:
                    topk = torch.topk(weights, k=3)
                    topk_indices = topk.indices.cpu().numpy()
                    topk_weights = topk.values.cpu().detach().numpy()
                    # Save to run_id subdirectory in the output directory
                    topk_dir = os.path.join(output_dir, str(run_id))
                    os.makedirs(topk_dir, exist_ok=True)
                    topk_path = os.path.join(topk_dir, "topk_slices_test.txt")
                    with open(topk_path, "a") as f:
        ...
        torch.save(state_dict_to_save, epoch_ckpt_path)
        ...
        if f1 > best_val_f1:
            ...
            best_epoch = epoch
            epochs_no_improve = 0
            # Save the best model's state_dict (handle DataParallel)
            if isinstance(model, nn.DataParallel):
